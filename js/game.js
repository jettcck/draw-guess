...
function handleDrawingStart() {
  ...
  G.correctGuessers = new Set();  // 新一轮重置猜对列表
  stopTimer();
  clearCanvasLocal();
  ...
  // 如果是绘画者，不需要启动计时器（在选词后才启动）
  if (G.isDrawer) {
    if (G.gameStatus === 'drawing' && G.currentWord) {
      // 已经在画了
      startRoundTimer();
    }
  } else {
...
function startRoundTimer() {
  startTimer(G.roundTime, null, async () => {
    // 时间到
    if (G.isDrawer) {
      // 只有绘画者负责推进状态
      await gameDb
